...
makeDefaultPolicy()
{
//  ProbCacheExtPolicy pcep = new ProbCacheExtPolicy();
  return unique_ptr<Policy>(new ProbCacheExtPolicy::ProbCacheExtPolicy());
}
 ...
 */
std::map<std::string, ProbCacheExtDataInfo*> Cs::getDataInfoMap() {
  return dataInfoMap;
}

/** \brief Getter for the map
 */
void Cs::setDataInfoMap(std::map<std::string, ProbCacheExtDataInfo*> map) {
  dataInfoMap = map;
}
