...
#include "../table/probext-data-info.hpp"
...
//  , m_decisionPolicy(new nfd::cs::UniformDecisionPolicy(100))
  , m_decisionPolicy(new nfd::cs::ProbCacheExtDecisionPolicy())
{
...
Forwarder::onIncomingInterest(Face& inFace, const Interest& interest)
{
  ...
  // Code for maintaining Data frequency
  Name dataName = interest.getName();
  std::string nameUri = dataName.toUri();
//  std::cout << "Name URI: " << dataName.toUri() << "\n" << std::flush;
  int dataFrequency = 0;
//  std::cout << "Setting Data Frequency\n" << std::flush;
  if(nameUri.find("client") != std::string::npos) {
    if(!m_decisionPolicy->dataInfoMap.count(nameUri)) {
      m_decisionPolicy->dataInfoMap.insert(std::make_pair(nameUri, new ProbCacheExtDataInfo()));
      m_decisionPolicy->dataInfoMap.find(nameUri)->second->setFrequency(1);
    } else {
      dataFrequency = m_decisionPolicy->dataInfoMap.find(nameUri)->second->getFrequency();
    //  std::cout << "dataFrequency: " << dataFrequency << "\n" << std::cout;
      m_decisionPolicy->dataInfoMap.find(nameUri)->second->setFrequency(++dataFrequency);
    } 
  }

  m_cs.setDataInfoMap(m_decisionPolicy->dataInfoMap);
                             ...
                             const Data& data)
{
  ...
  // Block of code to get and set the time-since-inception from the hop-count of interest packet
  int hopCount = 0;
  ...
  if (ns3PacketTag != nullptr) { // e.g., packet came from local node's cache
    ...
    NS_LOG_DEBUG("ns3packettag is null");
  }
//  const int hopCount_t = hopCount;
  const_cast<Data&>(data).setTSI(hopCount);
//  data.setTSI(hopCount);
...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  // Code for calculating number of Requests
  uint64_t numOfRequests;
//  std::cout << "m_counters.getNInInterests: " << m_counters.getNInInterests() << "\n" << std::flush;
  if(m_counters.getNInInterests() < 4) {
    numOfRequests = 0;
  } else {
    numOfRequests = m_counters.getNInInterests() - 4;
  }
//  std::cout << "numOfRequests: " << numOfRequests << "\n" << std::flush;

  // Code for getting the data name
  Name dataName = data.getName();
  std::string nameUri = dataName.toUri();

  // Finding the number of Requests for each content
  /*for (std::map<std::string, ProbCacheExtDataInfo*>::iterator it=m_decisionPolicy->dataInfoMap.begin(); it!=m_decisionPolicy->dataInfoMap.end(); ++it)
...
//  std::cout << "nameUri: " << nameUri << " dataFrequency: " << dataFrequency << "\n" << std::flush;
  m_decisionPolicy->setNumOfRequests(dataFrequency);
  m_decisionPolicy->setTotalRequests(numOfRequests); 
  ...
  // Setting the Interest so that hop-count may be extracted in the decision policy
  //m_decisionPolicy->setInterest(m_pit.find(dataName)->GetInterest());
  ...
  // CS insert
//  std::cout << "numOfRequests: " << numOfRequests << "\n" << std::flush;
//  if(numOfRequests < 1000) {
    if (m_decisionPolicy->admitPacket(data)) {
//    std::cout<< "Number of Requests is : "<<numOfRequests<<std::endl;
//    std::cout << "I am not hub: Data admisible!\n" << std::flush; 
      if (m_csFromNdnSim == nullptr) {
    //    std::cout << "I am not hub: Data admisible: NFD Content Store!\n" << std::flush; 
...
/*  } else {
    if (m_csFromNdnSim == nullptr) {
//      std::cout<< "Number of Requests is : "<<numOfRequests<<std::endl;
//      std::cout << "I am hub: Data admisible!\n" << std::flush; 
      m_cs.insert(*dataCopyWithoutPacket);
    } else {
      m_csFromNdnSim->Add(dataCopyWithoutPacket);
    }
